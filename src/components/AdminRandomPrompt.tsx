...
export default function AdminRandomPrompt({ onApply, onApplyImage1, onApplyImage2, currentImage2 }: Props) {
  ...
  const setImage1 = (file: File) => { setHeldImage1(file); onApplyImage1?.(file); setReference1File(file); };
  ...
  const refreshBounds = async () => {
    try {
      ...
      setLibrary((prev) => {
        prev.forEach((item) => { if (item.previewUrl && item.previewUrl.startsWith('blob:')) URL.revokeObjectURL(item.previewUrl); });
        return cards;
      });
    } catch (err: any) {
