...
function findLightboxImg(): HTMLImageElement | null {
  ...
  const visible = imgs.filter((img) => {
    ...
    return r.width > 220 && r.height > 220 && r.top >= -20 && r.bottom <= window.innerHeight + 80;
  });
  visible.sort((a, b) => {
...
export default function RegionEditRoot() {
  ...
  return (
    <>
      <canvas ref={canvasRef} className="pointer-events-none fixed z-[80]" style={{ left: 0, top: 0 }} />
      {hasLightbox && (
        <button
          type="button"
          data-arx-region-ui
