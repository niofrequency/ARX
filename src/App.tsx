...
// --- Native IndexedDB Wrapper (Unlimited Storage & Persistence) ---
...
export default function App() {
  ...
  const runBackgroundTask = async (id: string, base64: string, appMode: AppMode, execPrompt: string, resolution: Resolution) => {
    try {
      let finalUrl = '';
      if (appMode === 'upscaler') finalUrl = await triggerWavespeedUpscale(id, base64, resolution);
      else finalUrl = await triggerWavespeed(id, base64, execPrompt);
    ...
    <div className="min-h-screen bg-bg text-text-primary font-sans flex flex-col selection:bg-accent/20">
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto px-6 py-12 grid grid-cols-1 lg:grid-cols-12 gap-12">
        <div className="lg:col-span-5 space-y-10">
          <div className="flex bg-black/20 p-1.5 rounded-2xl border border-white/5 shadow-inner">
            <button onClick={() => setMode('editor')} className={`flex-1 py-3.5 rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all ${mode === 'editor' ? 'bg-accent text-bg shadow-[0_0_15px_rgba(0,242,255,0.3)]' : 'text-text-secondary hover:text-white'}`}><Sparkles className="w-4 h-4 inline mr-2" /> Editor</button>
            <button onClick={() => setMode('upscaler')} className={`flex-1 py-3.5 rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all ${mode === 'upscaler' ? 'bg-accent text-bg shadow-[0_0_15px_rgba(0,242,255,0.3)]' : 'text-text-secondary hover:text-white'}`}><Maximize className="w-4 h-4 inline mr-2" /> Upscaler</button>
          </div>
          <section>
          ...
          <section>
            ...
            <div className="space-y-6">
              ...
              <button disabled={!selectedFile} onClick={generateEdit} className="w-full py-5 rounded-2xl font-bold uppercase tracking-[0.2em] text-xs transition-all bg-accent text-bg hover:shadow-[0_0_30px_rgba(0,242,255,0.4)] disabled:opacity-50">Queue {mode === 'upscaler' ? 'Enhancement' : 'AI Edit'} Task</button>
            </div>
          </section>
        </div>
                  ...
                  <motion.div key={activeViewItem.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="w-full h-full p-4">
                    ...
                    {activeViewItem.status === 'completed' && activeViewItem.mode === 'upscaler' ? (
                      ...
                      <div className="w-full h-full cursor-pointer relative rounded-[2rem] overflow-hidden" onClick={() => { setSelectedHistoryItem(activeViewItem); setIsFlipped(false); }}>
                        ...
                        <div className="absolute inset-0 flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity duration-300"><div className="bg-black/60 px-4 py-2 rounded-full border border-white/10 shadow-xl backdrop-blur-sm"><span className="text-[10px] font-bold text-white uppercase tracking-widest">Click to Expand</span></div></div>
                      </div>
                    ) : activeViewItem.status === 'failed' && <div className="text-red-500 font-mono text-xs uppercase">Task Failed</div>}
                  </motion.div>
      ...
      {history.length > 0 && (
        <section className="max-w-6xl w-full mx-auto px-6 pt-16 border-t border-border/50 pb-12">
          <h2 className="text-xs font-bold uppercase tracking-[0.2em] text-text-secondary font-mono mb-8 flex items-center justify-between"><span>03 // Action Log (Unlimited)</span><span className="bg-accent/10 text-accent px-3 py-1 rounded-full">{history.length} Assets</span></h2>
