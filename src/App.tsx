...
{/* === WORKING SINGLE CARD FLIP MODAL (from your old reliable code) === */}
<AnimatePresence>
  {selectedHistoryItem && (
    <>
      <motion.div
      ...
      <div
        className="fixed inset-0 z-[90] flex items-center justify-center p-4 md:p-10 overflow-hidden touch-none"
      ...
      >
        ...
        {history.length > 1 && (
          <>
            <button
              ...
              className="hidden sm:flex fixed left-6 md:left-12 top-1/2 -translate-y-1/2 z-[100] w-14 h-14 rounded-full bg-white/10 border border-white/20 items-center justify-center text-white/60 hover:text-white hover:bg-white/20 transition-all"
            >
              <ChevronLeft className="w-8 h-8" />
            </button>
            <button
              onClick={(e) => { e.stopPropagation(); handleNextHistory(); }}
              className="hidden sm:flex fixed right-6 md:right-12 top-1/2 -translate-y-1/2 z-[100] w-14 h-14 rounded-full bg-white/10 border border-white/20 items-center justify-center text-white/60 hover:text-white hover:bg-white/20 transition-all"
            >
        ...
        <div className="relative w-full h-full max-w-5xl flex items-center justify-center" style={{ perspective: '2000px' }}>
          <div
            className="relative w-fit max-w-[92vw] max-h-[88vh] flex flex-col"
            style={{ perspective: '2000px', touchAction: 'none' }}
          >
            <motion.div
              className="relative shadow-2xl rounded-3xl cursor-pointer overflow-hidden"
              style={{ transformStyle: 'preserve-3d' }}
              animate={{ rotateY: isFlipped ? 180 : 0 }}
              transition={{ duration: 0.65, type: 'spring', stiffness: 280, damping: 26 }}
              onClick={(e) => {
                e.stopPropagation();
                setIsFlipped(!isFlipped);
              }}
            >
              {/* FRONT - MEDIA */}
              <div
                className="relative backface-hidden rounded-3xl overflow-hidden bg-black"
              ...
              >
                ...
                <button
                  ...
                  className="absolute top-4 right-4 z-50 p-3 bg-black/70 hover:bg-black rounded-2xl text-white"
                >
                  <X className="w-5 h-5" />
                </button>
                <button
                  onClick={(e) => { e.stopPropagation(); handleDeleteHistory(selectedHistoryItem.id); }}
                  className="absolute top-4 left-4 z-50 p-3 bg-black/70 hover:bg-red-900 rounded-2xl text-red-400"
                >
                  <Trash2 className="w-5 h-5" />
                </button>

                {/* Tap Hint */}
                <div className="absolute bottom-8 left-1/2 -translate-x-1/2 bg-black/70 px-6 py-3 rounded-2xl text-xs text-white/90 pointer-events-none">
                  TAP TO FLIP →
                </div>
              </div>
              ...
              {/* BACK - INFO */}
              <div
                className="absolute inset-0 backface-hidden rounded-3xl bg-zinc-950 flex flex-col overflow-hidden"
              ...
              >
                <div className="flex-1 p-6 sm:p-10 overflow-y-auto">
                  ...
                  {selectedHistoryItem.modelInfo && (
                    <p className="text-center text-[10px] text-zinc-500 mb-6 font-mono tracking-widest">
                      {selectedHistoryItem.modelInfo}
                    </p>
                  )}

                  <p className="text-zinc-100 text-[15px] leading-relaxed text-center">
                ...
                <div className="p-6 border-t border-zinc-800 bg-zinc-900 space-y-3">
                  <button
                    ...
                    className="w-full py-4 bg-white text-black rounded-2xl font-medium flex items-center justify-center gap-3 active:scale-95"
                  >
